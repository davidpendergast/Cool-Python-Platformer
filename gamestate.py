import pygame, sys, phys_objects
...
class LevelManager:
	...
	def load_level(self, num, actor, group):
		...
		try:
			...
			print file_name+" threw unexpected error:", sys.exc_info()[0]
			self.load_void_level(actor,group)
	def update_level_highscore(self, level_num, time):
		if len(self.level_highscore_data)-1 <= level_num:
			...
			if self.level_highscore_data[level_num][0] > time:
				...
				self.level_highscore_data[level_num] = (time, "New_name")
	def load_void_level(self, actor, group):
		...
		group.add(phys_objects.FinishBlock(16,16).set_xy(32+16+8,-64))
	...
	def write_header(self):
		header = open(self.file_dir+"/headerdump.txt", 'w')
		header.flush()
		header.write("@levels\t(filename)\n")
		
		for line in self.level_filenames:
			header.write(line+"\n")
		header.write("\n@highscores	(time, initials, formatted time, level)\n")
		i = 0
		for line in self.level_highscore_data:
			header.write(str(i)+"\t"+str(line[0])+"\t"+line[1]+"\n")
			i += 1
